...
class StockMovement(tk.Toplevel):
    ...
    def build_ui(self):
        # Create the LabelFrame container for the movement form.
        frm_form = ttk.LabelFrame(self, text="Record Movement")
        frm_form.pack(padx=10, pady=10, fill="x")

        self.lbl_product = ttk.Label(frm_form, text="Product")
        self.lbl_product.grid(row=0, column=0, sticky="w", padx=5, pady=5)
        self.combo_product = ttk.Combobox(frm_form, textvariable=self.var_product, state="readonly", width=28)
        self.combo_product["values"] = [f"{p[1]} - {p[2]}" for p in self.products]
        self.combo_product.grid(row=0, column=1, padx=5, pady=5)

        self.lbl_type = ttk.Label(frm_form, text="Type")
        self.lbl_type.grid(row=1, column=0, sticky="w", padx=5, pady=5)
        self.combo_type = ttk.Combobox(frm_form, textvariable=self.var_type, state="readonly", width=28)
        self.combo_type["values"] = ["IN", "OUT"]
        self.combo_type.grid(row=1, column=1, padx=5, pady=5)

        self.lbl_quantity = ttk.Label(frm_form, text="Quantity")
        self.lbl_quantity.grid(row=2, column=0, sticky="w", padx=5, pady=5)
        self.txt_quantity = ttk.Entry(frm_form, textvariable=self.var_quantity, width=30)
        self.txt_quantity.grid(row=2, column=1, padx=5, pady=5)

        frm_actions = ttk.Frame(self)
        frm_actions.pack(padx=10, pady=(0, 10), fill="x")
        self.btn_save = ttk.Button(frm_actions, text="Save", command=self.on_save_movement)
    ...
    def on_save_movement(self):
        try:
            ...
            clean = inventory_manager.MovementValidator.validate(pid, self.var_type.get(), self.var_quantity.get(), current_stock)
